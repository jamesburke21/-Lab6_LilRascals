...
library(DALEX2)
COflights <- read_csv("https://files.osf.io/v1/resources/3z5as4pej8/providers/osfstorage/5a8ca28f57103100104584db")
late <- COflights %>% filter(!is.na(ARR_TIME)) %>% filter(ARR_DELAY >= 15 & MONTH == 6) %>% summarise(count = n())
total <- COflights %>% filter(!is.na(ARR_TIME)) %>% summarise(count = n())
prob <- late/total
prob
ggplot(data = prob, aes(x = MONTH)) +
